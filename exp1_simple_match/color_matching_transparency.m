...
% monxyY = dlmread('D:\christophw\2018-9 BA\Melanie\exp_colourtouch1b\calibration\2018.10.29 calib Eizo 10bit\coltouch_20181029_10bit.xyY');
% monxyY = monxyY(1:3,1:3);
...
try
    ...
    for tc = st:length(trialOrder)
        ...
        while ~trialDone
            ...
            r = sqrt(((xm - width/2)/width)^2.0 + ((ym - height/2)/height)^2.0);
            if r > 1.0
                r = 1.0;
            end

            theta = atan2((ym - height/2)/height, (xm - width/2)/width);
            rgb = dklcart2rgb([ld, r*cos(theta), r*sin(theta)]);
            ...
            elseif strcmp(KbName(keyCode), 'space') || buttons(2)
                data(tc,1,:) = rgb;
                data(tc,2,:) = [ld, r*cos(theta), r*sin(theta)];
                data(tc,3,:) = [r, theta, 0];
                trialDone = true;
        ...
        if ~strcmp(subID, 'test')
            save(['data/color_match_transparency_' subID '_' condition '.mat']);
        end
