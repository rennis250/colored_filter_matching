...
for x = 1:length(conditions)
    ...
    fns = dir(['data/exp2/*_' condition '.mat']);
    ...
    for x = 1:length(ims)
        ...
        if strcmp(illumn, 'blue')
            illum_color_for_plot(xc, :) = object_colors(1, :);
            which_illum(xc) = 1;
        elseif strcmp(illumn, 'yellow')
            illum_color_for_plot(xc, :) = object_colors(2, :);
            which_illum(xc) = 2;
        ...
        if strcmp(refln, 'blue')
            if strcmp(ld, 'lighter')
                trans_color_for_plot(xc, :) = object_colors(1, :);
                which_trans(xc) = 1;
            else
                trans_color_for_plot(xc, :) = object_colors(5, :);
                which_trans(xc) = 5;
            end
        elseif strcmp(refln, 'yellow')
            if strcmp(ld, 'lighter')
                trans_color_for_plot(xc, :) = object_colors(2, :);
                which_trans(xc) = 2;
            else
                trans_color_for_plot(xc, :) = object_colors(6, :);
                which_trans(xc) = 6;
            end
        elseif strcmp(refln, 'red')
            if strcmp(ld, 'lighter')
                trans_color_for_plot(xc, :) = object_colors(3, :);
                which_trans(xc) = 3;
            else
                trans_color_for_plot(xc, :) = object_colors(7, :);
                which_trans(xc) = 7;
            end
        elseif strcmp(refln, 'green')
            if strcmp(ld, 'lighter')
                trans_color_for_plot(xc, :) = object_colors(4, :);
                which_trans(xc) = 4;
            else
                trans_color_for_plot(xc, :) = object_colors(8, :);
                which_trans(xc) = 8;
            end
        end
        xc = xc + 1;
    ...
    for fc = 1:length(fns)
        ...
        clear subID;
        clear trialOrder;
        load(['data/exp2/' fns(fc).name], 'data', 'subID', 'trialOrder', 'ntrials');
        ...
        for tc = 1:size(ims, 1)
            ...
            data_lab(fc, c, :, :) = zeros(ntrials, size(data, 3));
            for zc = 1:length(idxs)
                data_lab(fc, c, zc, :) = real(rgb2lab(squeeze(data(idxs(zc), 1, :))', monxyz));
                data_rgb(fc, c, zc, :) = real(squeeze(data(idxs(zc), 1, :))');
            end
            avgdata_lab(fc, c, :) = real(mean(data_lab(fc, c, :, :), 3));
            avgdata_rgb(fc, c, :) = real(mean(data_rgb(fc, c, :, :), 3));
    ...
    for ic = 1:size(imgs, 1)
        ...
        obj_lab(ic, :, :) = rgb2lab(squeeze(obj_rgb(ic, :, :)), monxyz);
        ...
        obj_lab_wout_high(ic, :, :) = rgb2lab(squeeze(obj_rgb_wout_high(ic, :, :)), monxyz);
        ...
        obj_lab_wout_refls(ic, :, :) = rgb2lab(squeeze(obj_rgb_wout_refls(ic, :, :)), monxyz);
        ...
        refls_lab(ic, :, :) = rgb2lab(squeeze(refls_rgb(ic, :, :)), monxyz);
        ...
        high_lab(ic, :, :) = rgb2lab(squeeze(high_rgb(ic, :, :)), monxyz);
        mean_high_rgb(ic, :) = mean(squeeze(high_rgb(ic, :, :)), 1);
        mean_high_lab(ic, :) = mean(squeeze(high_lab(ic, :, :)), 1);

        lum_regions = linspace(0, squeeze(max(max(obj_lab(ic, :, 1)))), 5);
        lum_regions_wout_refls = linspace(0, squeeze(max(max(obj_lab_wout_refls(ic, :, 1)))), 5);

        % next, let's try to get regions of different luminance and see what the mean colors are
        c = 1;
        for lc = 1:length(lum_regions)-1
            ls = squeeze(obj_lab_wout_high(ic, :, 1));
            lab_lum_splits{ic, c} = squeeze(obj_lab_wout_high(ic, ls >= lum_regions(lc) & ls < lum_regions(lc+1), :));
...
set(l, 'LineWidth', 2);
set(l, 'LineStyle', '--');
m = min(min([mean_obj_lab_wout_high(:,1) mean_obj_lab_wout_refls(:,1)]));
axis([0 100 0 100]);
axis square;
p1 = scatter(mean_obj_lab_wout_high(which_illum == 1, 1), popavgdata_lab(which_illum == 1, 1), [], trans_color_for_plot(which_illum == 1, :), 'o', 'filled');
% p1.MarkerEdgeColor = object_colors(1, :);
p2 = scatter(mean_obj_lab_wout_high(which_illum == 2, 1), popavgdata_lab(which_illum == 2, 1), [], trans_color_for_plot(which_illum == 2, :), 'v', 'filled');
% p2.MarkerEdgeColor = object_colors(2, :);

% p3 = scatter(mean_obj_lab_wout_refls(which_illum == 1, 1), popavgdata_lab(which_illum == 1, 1), [], trans_color_for_plot(which_illum == 1, :), 'o', 'filled');
% p3.MarkerEdgeColor = object_colors(1, :);
% p4 = scatter(mean_obj_lab_wout_refls(which_illum == 2, 1), popavgdata_lab(which_illum == 2, 1), [], trans_color_for_plot(which_illum == 2, :), 'v', 'filled');
% p4.MarkerEdgeColor = object_colors(2, :);

% legend([p1, p2], 'Without highlight included', 'Without specular reflections','Location','SouthEast');
...
p1 = scatter(mean_obj_lab_wout_high(which_illum == 1, 2), popavgdata_lab(which_illum == 1, 2), [], trans_color_for_plot(which_illum == 1, :), 'o', 'filled');
% p1.MarkerEdgeColor = object_colors(1, :);
p2 = scatter(mean_obj_lab_wout_high(which_illum == 2, 2), popavgdata_lab(which_illum == 2, 2), [], trans_color_for_plot(which_illum == 2, :), 'v', 'filled');
% p2.MarkerEdgeColor = object_colors(2, :);

% p3 = scatter(mean_obj_lab_wout_refls(which_illum == 1, 2), popavgdata_lab(which_illum == 1, 2), [], trans_color_for_plot(which_illum == 1, :), 'o', 'filled');
% p3.MarkerEdgeColor = object_colors(1, :);
% p4 = scatter(mean_obj_lab_wout_refls(which_illum == 2, 2), popavgdata_lab(which_illum == 2, 2), [], trans_color_for_plot(which_illum == 2, :), 'v', 'filled');
% p4.MarkerEdgeColor = object_colors(2, :);

l = line([-100 100], [-100 100]);
set(l, 'LineWidth', 2);
set(l, 'LineStyle', '--');
m = min(min([mean_obj_lab_wout_high(:,2) mean_obj_lab_wout_refls(:,2)]));
axis([-20 100 -20 100]);
axis square;
subplot(1,3,3);
hold on;
...
p1 = scatter(mean_obj_lab_wout_high(which_illum == 1, 3), popavgdata_lab(which_illum == 1, 3), [], trans_color_for_plot(which_illum == 1, :), 'o', 'filled');
% p1.MarkerEdgeColor = object_colors(1, :);
p2 = scatter(mean_obj_lab_wout_high(which_illum == 2, 3), popavgdata_lab(which_illum == 2, 3), [], trans_color_for_plot(which_illum == 2, :), 'v', 'filled');
% p2.MarkerEdgeColor = object_colors(2, :);

% p3 = scatter(mean_obj_lab_wout_refls(which_illum == 1, 3), popavgdata_lab(which_illum == 1, 3), [], trans_color_for_plot(which_illum == 1, :), 'o', 'filled');
% p3.MarkerEdgeColor = object_colors(1, :);
% p4 = scatter(mean_obj_lab_wout_refls(which_illum == 2, 3), popavgdata_lab(which_illum == 2, 3), [], trans_color_for_plot(which_illum == 2, :), 'v', 'filled');
% p4.MarkerEdgeColor = object_colors(2, :);

l = line([-100 100], [-100 100]);
set(l, 'LineWidth', 2);
set(l, 'LineStyle', '--');
m = min(min([mean_obj_lab_wout_high(:,3) mean_obj_lab_wout_refls(:,3)]));
axis([-5 100 -5 100]);
axis square;
export_fig('~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/mean_match_obj_correlation.pdf', '-pdf');

% plot showing lum regions with best match
figure(5);
clf;
hold on;
for sc = 1:size(best_lum_split, 1)
    bs = [length(find(best_lum_split(sc, :) == 1)) length(find(best_lum_split(sc, :) == 2)) length(find(best_lum_split(sc, :) == 3))];
...
export_fig('~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/hist_lum_regions.pdf', '-pdf');
...
export_fig('~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/hist_dist_matches.pdf', '-pdf');
...
for ic = 1:size(imgs, 1)
    ...
    imwrite(reshape(curr_img_gray_col, size(curr_img,1), size(curr_img,2), size(curr_img,3)), ['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/de_5_units/' fn(1:end-4) '_de_5.png']);

    lum_regions = linspace(0, squeeze(max(max(obj_lab(ic, :, 1)))), 4);
    ...
    for lc = 1:length(lum_regions)-1
        ...
        imwrite(reshape(curr_img_gray_col, size(curr_img,1), size(curr_img,2), size(curr_img,3)), ['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/lum_regions/' fn(1:end-4) '_lum_region_' num2str(lc) '.png']);
    end
end
...
for ic = 1:size(imgs, 1)
    ...
    imwrite(reshape(curr_img_gray_col, size(curr_img,1), size(curr_img,2), size(curr_img,3)), ['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/de_5_units_wout_refls/' fn(1:end-4) '_de_5.png']);

    lum_regions_wout_refls = linspace(0, squeeze(max(max(obj_lab_wout_refls(ic, :, 1)))), 4);
    ...
    for lc = 1:length(lum_regions_wout_refls)-1
        ...
        imwrite(reshape(curr_img_gray_col, size(curr_img,1), size(curr_img,2), size(curr_img,3)), ['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/lum_regions_wout_refls/' fn(1:end-4) '_lum_region_' num2str(lc) '.png']);
    end
end
...
for ic = 1:size(imgs, 1)
    ...
    if do_big_plot
        ...
        fnparts = strsplit(ims(ic).name, '_');
        if strcmp(fnparts{5}, 'lightest.png')
           illum = fnparts{7};
        ...
        export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/illum_' illum '_transmission_' transmission '_obj.pdf'], '-pdf');
        ...
        export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/illum_' illum '_transmission_' transmission '_obj_refls.pdf'], '-pdf');
        ...
        h_leg = legend([p_obj_pts, p_obj, p_refl_pts, p_refl, p_high_pts, p_high], {'Object', 'Mean Object', 'Specular reflections', 'Mean specular reflections', 'Highlight', 'Mean Highlight'}, 'Location', 'NorthEastOutside');
        ...
        export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/illum_' illum '_transmission_' transmission '_obj_refls_high.pdf'], '-pdf');
        ...
        for sc = 1:size(avgdata_lab, 1)
            p_matches = scatter(avgdata_lab(sc, ic, 2), avgdata_lab(sc, ic, 3), [], squeeze(avgdata_rgb(sc, ic, :).^(1/2.2))', 12, 'filled');
        end

        h_leg = legend([p_obj_pts, p_obj, p_refl_pts, p_refl, p_high_pts, p_high, p_matches], {'Object', 'Mean Object', 'Specular reflections', 'Mean specular reflections', 'Highlight', 'Mean Highlight', 'Obs. matches'}, 'Location', 'NorthEastOutside');
        ...
        export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/illum_' illum '_transmission_' transmission '_obj_refls_high_obs_matches.pdf'], '-pdf');
        ...
        h_leg = legend([p_obj_pts, p_obj, p_refl_pts, p_refl, p_high_pts, p_high, p_matches, p_match], {'Object', 'Mean Object', 'Specular reflections', 'Mean specular reflections', 'Highlight', 'Mean Highlight', 'Obs. matches', 'Grand mean matches'}, 'Location', 'NorthEastOutside');
        ...
        export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/illum_' illum '_transmission_' transmission '_obj_refls_high_obs_matches_grand_mean.pdf'], '-pdf');
    end
end
...
for ic = 1:size(imgs, 1)
    ...
    if do_big_plot
        ...
        h_leg = legend([p_obj_pts, p_obj, p_matches, p_match], {'Object', 'Mean Object', 'Obs. matches', 'Grand mean'}, 'Location', 'NorthEastOutside');
        ...
        for sc = 1:size(avgdata_lab, 1)
            p_matches = scatter(avgdata_lab(sc, ic, 2), avgdata_lab(sc, ic, 1), [], squeeze(avgdata_rgb(sc, ic, :).^(1/2.2))', 12, 'filled');
        end
        ...
        export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/illum_' illum '_transmission_' transmission '_obj_high_obs_match.pdf'], '-pdf');
    end
end
...
imwrite(imtom, '~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/image_to_mask.png');
...
hold on;
imshow(imtos);
imwrite(imtos, '~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/image_masked_high.png');
...
hold on;
imshow(imtos);
imwrite(imtos, '~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/image_masked_obj.png');
...
hold on;
imshow(imtos);
imwrite(imtos, '~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/images/image_masked_refl.png');
...
empty_lab = rgb2lab(empty_rgb, monxyz);
...
for ic = 1:length(neutral_imgs)
    ...
    curr_lab = rgb2lab(curr_rgb, monxyz);
    ...
    export_fig(['~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/' neutral_imgs(ic).name(1:end-4) 'convergence_shift.pdf'], '-pdf');
end
...
bkg_lab = rgb2lab(bkg_rgb, monxyz);
...
export_fig('~/Dropbox/transparent_color_skin_of_things_nov_2018/exp2/plots/background_chromaticities.pdf', '-pdf');
...
trans_colors = {[0 0 1], [0.5 0.8 0.1], [1 0 0], [0 1 0], [0 0 0.5], [0.25, 0.4, 0.05], [0.5 0 0], [0 0.5 0]};
...
for ic = 1:size(imgs, 1)
    if which_illum(ic) == 1 % blue
        plot(avgdata_lab(:, ic, 2), avgdata_lab(:, ic, 3), 'o', 'MarkerEdgeColor', trans_colors{which_trans(ic)});
    end
end
...
for tc = 1:8
    ...
    line([popavgdata_lab(idxs(1), 2) popavgdata_lab(idxs(2), 2)], [popavgdata_lab(idxs(1), 3) popavgdata_lab(idxs(2), 3)]);
end

for ic = 1:size(imgs, 1)
    plot(popavgdata_lab(ic, 2), popavgdata_lab(ic, 3), 'o', 'MarkerEdgeColor', trans_colors{which_trans(ic)});
end
