basen <- "mitsuba_comp_rmc_rsd_lightest"

illumns <- c("blue", "yellow")
...
for(illumn in illumns) {
    for(by_mix in seq(-1,1,0.2)) {
    	for(rg_mix in seq(-1,1,0.2)) {
    		refl <- red
    		refl[,2] <- 0.5*(rg_mix*red[,2] + (1 - rg_mix)*green[,2]) + 0.5*(by_mix*blue[,2] + (1 - by_mix)*yellow[,2])
    		refln <- paste0("refls/rg_", rg_mix, "_by_", by_mix, ".spd")
            ...
            fileConn <- file(paste0("scenes/", basen, "_illum_", illumn, "_rg_", rg_mix, "_by_", by_mix, ".xml"))
