...
data <- list(read.csv('../exp1_simple_match/data/all_obs_data_together.csv'),
			read.csv('../exp2_dye_match/data/all_obs_data_together.csv'),
			read.csv('../exp3_filter_match/data/all_obs_data_together.csv'),
			read.csv('../exp11a_achromatic_w_patch/data/all_obs_data_together.csv'))
...
for (d in c("l", "a", "b")) {
	...
	abline(0, 1)
	if (d == "l") {
		mytitle(xlab = paste0("Image Mean ", toupper(d), "*"), ylab = paste0("Observer Match ", toupper(d), "*"))
	} else {
		mytitle(xlab = paste0("Image Mean ", d, "*"), ylab = paste0("Observer Match ", d, "*"))
	}
...
# plotting results for white bkg and illum in terms of mean LAB
pdf("../figures/white_wall_results.pdf", height = 3, width = 9)
